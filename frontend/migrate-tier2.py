...
def remove_dark_rules(content):
    ...
    for line in lines:
        # 检测是否是 :global(.dark) 规则的开始
        if re.search(r':global\(\.dark\)\s+.*\{', line):
            in_dark_block = True
...
# 迁移文件列表
files = [
    'src/views/MarkdownTool.vue',
    'src/views/MermaidTool.vue',
    'src/views/PasteBin.vue',
    'src/views/PasteView.vue',
    'src/views/MockApi.vue',
]

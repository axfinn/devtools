...
// TestProxyAnthropicGeneric_NonStreamAutoAsync 验证 /api/anthropic/v1/messages
// 非流式 POST 自动转异步模式(返 202 + Location 头 + task_id),防 CF 524 origin read timeout。
//
// 触发场景: Claude Code v2.1.235 在混合调用里会用非流式做后台探测(haiku summarization、
// compact 等),走 Ollama 慢响应会被 CF 15s cut。改后非流式自动转异步,
// POST 在 <500ms 返 202,后台跑 Ollama。
func TestProxyAnthropicGeneric_NonStreamAutoAsync(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstreamCalled := &atomic.Int32{}
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		upstreamCalled.Add(1)
		// 模拟 Ollama 慢响应 — 如果是同步路径,CF 会 524
		time.Sleep(100 * time.Millisecond)
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		io.WriteString(w, `{"id":"msg_auto","content":[{"type":"text","text":"ok"}]}`)
	}))
	defer upstream.Close()
	...
	router := gin.New()
	router.POST("/api/anthropic/v1/messages", h.ProxyAnthropicGeneric)
	router.GET("/api/anthropic/v1/messages/tasks/:id", h.GetAnthropicMessageTask)

	// 非流式 body(stream 缺省 = false)
	body := []byte(`{"model":"qwen3.8:27b-mlx","max_tokens":32,"messages":[{"role":"user","content":"hi"}]}`)

	start := time.Now()
	req := httptest.NewRequest(http.MethodPost, "/api/anthropic/v1/messages", bytes.NewReader(body))
	req.Header.Set("X-Super-Admin-Password", "test-admin-pw")
	req.Header.Set("Content-Type", "application/json")
	wr := httptest.NewRecorder()
	router.ServeHTTP(wr, req)
	...
	// 关键断言: 202 + Location 头 + POST 极快(<500ms 防 CF 524)
	if wr.Code != http.StatusAccepted {
		t.Fatalf("expected 202 Accepted, got %d; body: %s", wr.Code, wr.Body.String())
	}
	if loc := wr.Header().Get("Location"); loc == "" {
		t.Error("Location header should be set for async response")
	} else if !strings.HasPrefix(loc, "/api/anthropic/v1/messages/tasks/oant_") {
		t.Errorf("Location header malformed: %s", loc)
	}
	if postDuration > 500*time.Millisecond {
		t.Errorf("POST took %v, must be <500ms to avoid CF 524", postDuration)
	}
	...
	taskID, _ := resp["task_id"].(string)
	if !strings.HasPrefix(taskID, "oant_") {
		t.Errorf("task_id should have oant_ prefix, got: %s", taskID)
	}
	if resp["status"] != "pending" {
		t.Errorf("initial status = %v, want pending", resp["status"])
	}

	// 轮询直到 succeeded(后台 worker 应该已经跑完)
	deadline := time.Now().Add(3 * time.Second)
	var finalBody []byte
	for time.Now().Before(deadline) {
		req2 := httptest.NewRequest(http.MethodGet, "/api/anthropic/v1/messages/tasks/"+taskID, nil)
		req2.Header.Set("X-Super-Admin-Password", "test-admin-pw")
		wr2 := httptest.NewRecorder()
		router.ServeHTTP(wr2, req2)
		finalBody = wr2.Body.Bytes()
		var pr map[string]interface{}
		_ = json.Unmarshal(finalBody, &pr)
	...
	if finalResp["status"] != "succeeded" {
		t.Errorf("task should succeed via auto-async, got: %s", finalBody)
	}

	// 上游应该被调用了一次
	if upstreamCalled.Load() != 1 {
		t.Errorf("upstream should be called once, got %d", upstreamCalled.Load())
	}
}

// TestProxyAnthropicGeneric_StreamStillSync 验证 /api/anthropic/v1/messages
// 流式(stream:true)仍走同步流式路径(返 SSE),不被本次异步化改动影响。
// 防止 stream:true 被误判走异步路径。
func TestProxyAnthropicGeneric_StreamStillSync(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	cfg.AIGateway.SuperAdminPassword = "test-admin-pw"
	h := NewAIGatewayHandler(db, cfg, nil, nil, testEncEncryptionService)
	h.streamClient.Transport = &http.Transport{} // 默认 transport
	...
	// 流式应返 200(不是 202 异步),SSE 头 + content
	if wr.Code != http.StatusOK {
		t.Fatalf("stream should return 200, got %d; body: %s", wr.Code, wr.Body.String())
	}
	if ct := wr.Header().Get("Content-Type"); !strings.HasPrefix(ct, "text/event-stream") {
		t.Errorf("Content-Type should be text/event-stream, got: %s", ct)
	}
	// 流式响应不应有 task_id 字段(那是异步的)
	var resp map[string]interface{}
	_ = json.Unmarshal(wr.Body.Bytes(), &resp)
	if _, hasTaskID := resp["task_id"]; hasTaskID {
		t.Errorf("stream response should not contain task_id, got: %s", wr.Body.String())
	}
}

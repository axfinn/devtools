...
| 维度 | 设计 |
|------|------|
| 领域对象 | `Trip` 聚合根 → `DayPlan` 实体 → `Activity` 实体 + `Destination` 值对象 |
| 数据访问 | `Repository` 接口 + `SQLiteRepository` 实现,复用 `models.DB.Conn()` 同连接 |
| 用例层 | `trip.Service`:`CreateTrip / AddDay / AddActivity / SetReminder / RenderMarkdown / SeedRealTrip` |
...
| 前端入口 | `frontend/src/views/life/TripTool.vue`,路由 `/trip` |
...
| `DELETE` | `/api/trips/:id` | 级联删除 trip + days + activities |
...
```bash
go build -o /tmp/trip ./backend/cmd/trip && /tmp/trip --db /tmp/trip.db seed
/tmp/trip list
/tmp/trip show <trip-id>             # 完整 Markdown 到 stdout
/tmp/trip remind <activity-id> --before 60 --email foo@bar.com --reset
```

## 第一个真实行程

落地数据来自 `trip.SeedRealTrip`,把 issue DEVT-20 给的 14 天散文
重新组织成 DayPlan + Activity 结构,落到「国庆 2026 粤港澳潮汕 14 日深度游」。
数据库内 fixture 重跑即幂等。
